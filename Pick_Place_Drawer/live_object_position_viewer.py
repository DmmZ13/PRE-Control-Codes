...
em vez de capturar um unico frame, fica rodando continuamente,
detectando objetos e calculando a posicao 3D real (via profundidade)
a cada frame novo, mostrando tudo numa janela com overlay.

MUDANCA: agora usa a transformacao COMPLETA (rotacao + translacao)
obtida pela calibracao multi-ponto via Kabsch
(robot_to_table_transform.json), em vez de assumir soma direta
(so' translacao, presumindo eixos perfeitamente alinhados) -- a
calibracao multi-ponto revelou um pequeno desalinhamento de
rotacao (~2-3 graus) que a soma direta nao corrigia.
...
ROBOT_TRANSFORM_PATH = "robot_to_table_transform.json"
...
def world_to_robot_base(point_world, robot_rotation, robot_translation):
    """
    Converte uma posicao do referencial da MESA (mundo/ArUco) para o
    referencial da BASE DO ROBO, usando a transformacao COMPLETA
    (rotacao + translacao) obtida pela calibracao multi-ponto Kabsch:

        P_robo = R @ P_mesa + t
    """
    return robot_rotation @ point_world + robot_translation
...
class LiveObjectPositionViewer(Node):
    def __init__(self, class_names, camera_rotation_world, camera_position_world,
        ...
        self.robot_translation = robot_translation
    ...
    def process_and_draw(self):
        ...
        for box in results.boxes:
            ...
            if depth_m is None:
                text = f"{cls_name} {conf:.2f} | sem profundidade valida"
            else:
                ...
                point_robot_base = world_to_robot_base(
                    point_world, self.robot_rotation, self.robot_translation
                )

                print(f"Ponto world: {point_world}")
                print(f"Ponto robo:  {point_robot_base}")

                text = (f"{cls_name} {conf:.2f} | {depth_m:.2f}m | "
                        f"robo=({point_robot_base[0]:.3f},{point_robot_base[1]:.3f},{point_robot_base[2]:.3f})")

                # Publica a posicao no referencial do ROBO (usando a
                # transformacao completa Kabsch), para outros nos
                # (ex: controle do robo) consumirem
                point_msg = Point()
                point_msg.x = float(point_robot_base[0])
                point_msg.y = float(point_robot_base[1])
                point_msg.z = float(point_robot_base[2])
            ...
            cv2.putText(overlay, text, (int(x1), max(0, int(y1) - 5)),
...
def main():
    ...
    parser.add_argument("--classes", type=str, nargs="+", required=True, default=["mug", "cup"],
                         help="Lista de classes a detectar (ex: --classes cup mug bowl)")
    parser.add_argument("--pose-path", type=str, default=CAMERA_POSE_PATH)
    parser.add_argument("--robot-transform-path", type=str, default=ROBOT_TRANSFORM_PATH)
    ...
    try:
        ...
        print(f"[ERRO] {args.pose_path} nao encontrado -- rode "
              f"calibrate_camera_pose.py primeiro.")
        sys.exit(1)

    try:
        with open(args.robot_transform_path) as f:
            robot_transform_data = json.load(f)
    except FileNotFoundError:
        print(f"[ERRO] {args.robot_transform_path} nao encontrado -- rode "
              f"calibrate_robot_to_table_multipoint.py primeiro.")
        sys.exit(1)
    ...
    robot_rotation = np.array(robot_transform_data["rotation_matrix"])
    robot_translation = np.array(robot_transform_data["translation"])
    ...
    try:
        while rclpy.ok():
            ...
            if display_frame is not None:
                cv2.imshow("Deteccao + Posicao 3D ao vivo", display_frame)
