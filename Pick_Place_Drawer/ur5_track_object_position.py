...
# ── ORIENTACAO DE AGARRE DESEJADA (quaternion fixo) ──────────────────
# Em vez da abordagem horizontal, usamos uma orientacao FIXA definida
# por um quaternion [x, y, z, w]. O eixo Z desta matriz e' a direcao de
# abordagem: e' ao longo dele que o robo recua APPROACH_CLEARANCE_M para
# formar a linha de aproximacao ate' o objeto.
...
GRIPPER_IO_PIN = 16        # ajuste para o pino real da sua garra
GRIPPER_CLOSE_STATE = 1.0  # 1.0 = fecha, 0.0 = abre (confira polaridade da sua garra)
IO_SERVICE_NAME = "/io_and_status_controller/set_io"


def quaternion_to_rotation_matrix(q_xyzw: np.ndarray) -> np.ndarray:
    """Converte um quaternion [x, y, z, w] em matriz de rotacao 3x3.
    Normaliza antes, para tolerar pequenos erros de arredondamento."""
    x, y, z, w = q_xyzw
...
# Matriz de rotacao alvo, calculada UMA vez a partir do quaternion.
...
def compute_grasp_rotation(current_rotation: np.ndarray) -> np.ndarray:
    """Retorna a orientacao de agarre desejada (fixa, do quaternion).
    Mantem a assinatura antiga (recebe current_rotation) so' para nao
    quebrar a chamada em compute_once; o argumento e' ignorado, pois a
    orientacao agora e' fixa e nao depende da pose atual."""
...
class UR5ObjectPositionTracker(Node):
    ...
    def __init__(self):
        ...
        self.io_client = self.create_client(SetIO, IO_SERVICE_NAME)
    ...
    def close_gripper(self):
        if not self.io_client.service_is_ready():
            self.get_logger().warn("Servico de IO nao disponivel -- garra NAO fechada.")
            return

        req = SetIO.Request()
        req.fun = 1  # FUN_SET_DIGITAL_OUT (padrao do driver ur_robot_driver)
        req.pin = GRIPPER_IO_PIN
        req.state = GRIPPER_CLOSE_STATE

        future = self.io_client.call_async(req)
        future.add_done_callback(
            lambda f: self.get_logger().info("[GARRA] Comando de fechar enviado.")
        )
    ...
    def execute_move(self):
        ...
        # Agenda o fechamento da garra para quando o movimento terminar.
        # create_timer com one_shot nao existe direto na API antiga do
        # rclpy -- usamos um timer normal que se autocancela na primeira
        # execucao.
        def _trigger_gripper_close():
            self.close_gripper()
            self._gripper_timer.cancel()

        self._gripper_timer = self.create_timer(MOVE_DURATION_S, _trigger_gripper_close)
